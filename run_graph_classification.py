...
for key in datasets:
    ...
    elif args.rewiring == 'nnpr':
        for i in range(len(dataset)):
            if args.sample:
                dataset[i].edge_index, dataset[i].edge_type, dataset[i].edge_weight = nnpr.nnpr(
                ...
                dataset[i].edge_index, dataset[i].edge_type, dataset[i].edge_weight = nnpr.innpr(dataset[i].edge_index, iteration=args.num_iterations, depth=args.num_layers)
    ...
    results.append({
        ...
        "energy_ci": energy_ci,
        "sample": None if not args.sample else args.scale
        })
df = pd.DataFrame(results)
with open(f"./results/{args.logfile}", 'a') as f:
